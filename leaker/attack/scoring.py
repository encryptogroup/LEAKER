...
class SamplingScoringAttack(ScoringAttack):
    """
    Implements the Scoring attack from [DHP21]. Using sampling co-occurrence estimates.
    If known_query_size == 0, they will be uncovered like in [CGPR15]
    """
    ...
    def __init__(self, known: SQLRelationalDatabase, known_query_size: float = 0.15):
        self.__est_sampling = SamplingRelationalEstimator(known, known.parent())
        super(SamplingScoringAttack, self).__init__(known, known_query_size)

    @classmethod
    def name(cls) -> str:
        return "SamplingScoring"
...
class SamplingRefinedScoringAttack(RefinedScoringAttack):
    """
    Implements the refined Scoring attack from [DHP21]. Using naru co-occurrence estimates.
    If known_query_size == 0, they will be uncovered like in [CGPR15]
    """
    ...
    def __init__(self, known: SampledSQLRelationalDatabase, known_query_size: float = 0.15, ref_speed: int = 10):
        self.__est_sampling = SamplingRelationalEstimator(known, known.parent())
        super(SamplingRefinedScoringAttack, self).__init__(known, known_query_size, ref_speed)
        self._ref_speed = ref_speed

    @classmethod
    def name(cls) -> str:
        return "SamplingRefinedScoring"
