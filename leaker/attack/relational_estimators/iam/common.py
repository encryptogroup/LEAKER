...
class Column(object):
    ...
    def SetGMM(self, is_gmm=False, mm_num=-1):
        self.is_gmm = is_gmm
        self.mm_num = mm_num
...
class CsvTable(Table):
    ...
    def __init__(self,
        ...
        #self.origin_data = self.data.copy(deep=True)
        #for c in self.gmm_cols:
        #    self.origin_data[c].fillna(0, inplace=True)
    ...
    def _build_columns(self, data, cols, type_casts, pg_cols):
        ...
        for c, p in zip(cols, pg_cols):
            ...
            if c in self.gmm_cols:
                print('GMM_COLUMN:\t',c)
                #origin_data = data[c].tolist()
                is_nan = pd.isnull(data[c])
                contains_nan = np.any(is_nan)
                print(c, 'CONTAINS_NAN: ', contains_nan)
                col.SetGMM(is_gmm=True, mm_num=self.gmm_num[c])
                #tmp_data = [i % self.gmm_num[c] for i in range(len(data))] 
                #col.Fill(tmp_data)
                col.Fill(data[c])
                col.all_distinct_values = np.arange(self.gmm_num[c])
                col.distribution_size = self.gmm_num[c]
            else:
                col.Fill(data[c])
...
class FactorizedTable(Dataset):
    ...
    def _factorize(self, tuples_np):
        ...
        for i, col in enumerate(self.table_dataset.table.Columns()):
            ...
            if dom <= self.word_size or col.is_gmm is True:
                ...
                if col.is_gmm:
                    new_col.SetGMM(is_gmm = col.is_gmm, mm_num = col.mm_num)
                cols.append(new_col)
...
class TableDataset(Dataset):
    ...
    def __init__(self, table, input_encoding=None):
        ...
        for c in self.table.Columns():
            if c.name in self.table.gmm_cols:
                self.tuples_np.append(np.array(c.data))
            else:
                self.tuples_np.append(self.Discretize(c))
...
class FactorizedSampleFromJoinIterDataset(IterableDataset):
    ...
    def _factorize_columns(self):
        ...
        for i, col in enumerate(self.base_table_cols):
            dom = col.DistributionSize()
            if _should_not_factorize(col) or col.is_gmm is True:
                # Don't factorize this column.
                new_col = Column(col.name,
                                 distribution_size=col.distribution_size)
                new_col.SetDistribution(col.all_distinct_values)
                new_col.SetGMM(col.is_gmm, col.mm_num)
                cols.append(new_col)
    ...
    def _factorize_data(self, data):
        ...
        factorized_data = []
        for i, col in enumerate(self.base_table_cols):
            if col not in self.fact_col_mapping or col.is_gmm is True:
                # This column not factorized.
                #print('COL: ', col.name, col.is_gmm)
                #print(data[:, i])
                ...
                for fact_col in self.fact_col_mapping[col]:
                    ...
                    if num_bits < 0:
                        fact_data = data[:, i].astype(np.int) & (word_mask >> -num_bits)
                    else:
                        #print('COL: ', col.name, col.is_gmm)
                        #print('FACT_COL: ', fact_col.name, fact_col.is_gmm)
                        #print(data[:, i])
                        fact_data = (data[:, i].astype(np.int) >> num_bits) & word_mask
            ...
            for i, (col_name, col) in enumerate(
                ...
                if not self.disambiguate_column_names:
                    ...
                    assert col.name.endswith(col_name), (sampled_df.columns,
                                                         col_name, col.name)
                if col.is_gmm:
                    discretized_col_data = sampled_df.iloc[:, i].to_numpy(dtype=np.float, copy=True) 
                else:
