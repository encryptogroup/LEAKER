...
def _get_table_name(dataset):
    if dataset == 'imdb':
        table_list = [
            ...
            '__in_cast_info',
            '__in_movie_info',
            '__in_movie_companies']
        table_dict = {
            '__in_company_name': 'cn',
            '__in_keyword': 'k',
            '__in_company_type': 'ct',
            '__in_movie_keyword': 'mk',
            '__in_title': 't',
            '__in_movie_info_idx': 'mi_idx',
            '__in_cast_info': 'ci',
            '__in_movie_info': 'mi',
            '__in_movie_companies': 'mc'}
    elif dataset == 'higss':
        table_list = ['higss']
        table_dict = {'higss': 'y'}
    elif dataset == 'twitter':
        table_list = ['twitter']
        table_dict = {'twitter': 'y'}
    elif dataset == 'wisdm':
        table_list = ['wisdm']
        table_dict = {'wisdm': 'y'}
...
def UnpackQueries(concat_table, queries):
    ...
    for q in queries:
        ...
        if tables[0] != 'higss' and tables[0] != 'twitter' and tables[0] != 'wisdm':
            for table in tables:
        ...
        for table, preds in predicate_dict.items():
            ...
            for c, o, v in zip(cols, ops, vals):
                if tables[0] != 'higss' and tables[0] != 'twitter' and tables[0] != 'wisdm':
                    ...
                    column = concat_table.columns[concat_table.ColumnIndex(c)]
