...
def run_epoch(split,
    ...
    #if split == 'train':
    #    model.compute_mm_params(dataset)
    begin_time = t1 = time.time()

    for step, xb in enumerate(loader):
        #print(xb[0])
        data_time.update((time.time() - t1) * 1e3)
        #xb[torch.isnan(xb)] = 2e-30
        #print('HAS_NAN: ', torch.any(torch.isnan(xb)))
        if split == 'train':
        ...
        for i in range(num_orders_to_forward):
            ...
            model_in, model_out, mm_loss = model(xb)
            loss = mm_loss
        ...
        #s_xbhat = []
        #index_l = 0
        #index_u = model.input_bins[0]
        #for i in range(model.nin):
        #    data_i = model_in[:, index_l:index_u].nonzero()[:,1]
        #    s_xbhat.append(data_i)
        #    if i < model.nin - 1:
        #        index_l += model.input_bins[i]

        #print('MM_LOSS: ', loss)
        #s_xbhat = torch.cat(s_xbhat, dim=1)
        if num_orders_to_forward == 1:
            loss += model.nll(xbhat, model_in, label_smoothing=label_smoothing).mean()
        else:
...
def MakeMade(
    ...
    column_model = []
    id2column = [i for i in range(len(cols_to_train))]
    n_mm = []
    for c in cols_to_train:
        print('{}\t{}\t', c.name, c.is_gmm, c.mm_num)
        if c.is_gmm:
            column_model.append(1)
            n_mm.append(c.DistributionSize())
        else:
            column_model.append(0)
            n_mm.append(0)
    print('NIN: ', len(cols_to_train))
    print('INPUT_BINS: ', [c.DistributionSize() for c in cols_to_train])
    model = made.MADE(
...
class NeuroCard(tune.Trainable):
    ...
    def _setup(self, config):
        ...
        assert self.dataset in ['imdb', 'higss', 'twitter', 'wisdm']
        ...
        elif self.dataset == 'higss':
            table = datasets.LoadHigss()
        elif self.dataset == 'wisdm':
            table = datasets.LoadWisdm()
        elif self.dataset == 'twitter':
            table = datasets.LoadTwitter()
        ...
        '''
        path = '/mnt/e/linux/dataset/imdb/deepdb/job-light-ranges_mm_wisdm_deepdb.csv'
        table_names = ['title', 'cast_info', 'movie_info', 'movie_info_idx', 'movie_keyword', 'movie_companies']
        loaded_tables = []
        loaded_queries = []
        oracle_cards = []
        queries_job_format = utils.JobToQuery(path)
        loaded_queries, oracle_cards = utils.UnpackQueries(
                    table, queries_job_format)
        loaded_queries.append(loaded_queries)
        oracle_cards.append(oracle_cards)
        num_queries = len(loaded_queries)
        estimator = estimators_lib.Postgres('imdb', table_names, port=5432)
        errs = []
        start_time = time.time()
        for i in range(num_queries):
            query = loaded_queries[i]
            cols, ops, vals = query
            true_card = max(oracle_cards[i], 1)
            est_card = max(estimator.Query(cols, ops, vals), 1)
            err = max(true_card / est_card, est_card / true_card)
            errs.append(err)
        end_time = time.time()
        errs.sort()
        print("max", np.max(errs))
        print("99th", np.quantile(errs, 0.99))
        print("95th", np.quantile(errs, 0.95))
        print("median", np.quantile(errs, 0.5))
        print("mean", np.mean(errs))
        print("time(ms)", (end_time - start_time) / num_queries * 1000)
        exit()
        '''
        ...
        self.all_loaded_queries = []
        self.all_oracle_cards = []
        self.all_save_path = []
        card_dir = self.queries_csv + '_card'
        if os.path.exists(card_dir) is False:
            os.makedirs(card_dir)
        if (self.dataset == 'imdb' and len(self.join_tables) > 1) or self.dataset == 'higss' or self.dataset == 'twitter' or self.dataset == 'wisdm':
            #for f in os.listdir(self.queries_csv):
            for i in range(len(os.listdir(self.queries_csv))):
                f = 'q' + str(i) + '.csv'
                loadpath = os.path.join(self.queries_csv, f)
                card_filepath = os.path.join(card_dir, f)
                self.all_save_path.append(card_filepath)
                queries_job_format = utils.JobToQuery(loadpath)
                loaded_queries, oracle_cards = utils.UnpackQueries(
                    self.table, queries_job_format)
                self.all_loaded_queries.append(loaded_queries)
                self.all_oracle_cards.append(oracle_cards)
    ...
    def MakeOrdering(self, table):
        fixed_ordering = None
        if self.dataset != 'imdb' and self.dataset != 'higss' and self.dataset != 'wisdm' and self.dataset != 'twitter' and self.special_orders <= 1:
            ...
            if self.factorize:
                ...
                estimators.append(
                    estimators_lib.ProgressiveSampling(
                        ...
                        shortcircuit=dropout,
                        do_fanout_scaling=do_fanout_scaling))
            #table_names = ['title', 'movie_keyword', 'movie_companies', 'movie_info', 'movie_info_idx', 'cast_info'] 
            #estimators.append(estimators_lib.Postgres('imdb', table_names, 5433))
        return estimators
    ...
    def _train(self):
        ...
        if self.epoch == 0:
            loader = data.DataLoader(self.train_data,
                             batch_size=20480)
            init_data = next(iter(loader))
            init_data = init_data.float().to(train_utils.get_device(), non_blocking=True)
            print(init_data)
            self.model.compute_mm_params(init_data)
        for _ in range(min(self.epochs - self.epoch,
            ...
            mean_epoch_train_loss = run_epoch(
                ...
                self.model,
                self.opt,
                upto=self.max_steps if self.dataset == 'imdb' or self.dataset == 'higss' or self.dataset == 'twitter' or self.dataset == 'wisdm' else None,
        ...
        if self.compute_test_loss:
            returns['test_bits'] = np.mean(
                run_epoch(
                    ...
                    val_data=self.train_data,
                    batch_size=1024,
                    upto=None if self.dataset != 'imdb' and self.dataset != 'higss' and self.dataset != 'wisdm' and self.dataset != 'twitter' else 20,
                    log_every=200,
                    table_bits=self.table_bits,
                    return_losses=True,
                )) / np.log(2)
    ...
    def Query(self,
        ...
        table_list, table_dict = utils._get_table_name(self.dataset) 
        for est in estimators:
            ...
            using_cols = []
            for c in cols:
                if c.name in table_list:
                    table_alias = table_dict[c.name]
                    #query_card_str += (table_alias + ',')
                    using_cols.append(table_alias)
            all_perm_using_cols = list(itertools.permutations(using_cols))
            for perm in all_perm_using_cols:
                query_card_str = ''
                for c in perm:
                    query_card_str += (c + ',')
                query_card_str += (':' + str(est_card))
                self.save_fp.write(query_card_str + '\n')
    ...
    def evaluate(self, num_queries, done, estimators=None):
        ...
        model.gaussian_sampling(num=10000)
        results = {}
        if num_queries:
            ...
            assert self.all_loaded_queries is not None
            num_queries = min(len(self.all_loaded_queries), num_queries)
            for i in range(num_queries):
                self.save_fp = open(self.all_save_path[i], 'w')
                for j in range(len(self.all_loaded_queries[i])):
                    print('Subquery {} for Query{}:'.format(j, i), end=' ')
                    query = self.all_loaded_queries[i][j]
                    self.Query(estimators,
                           oracle_card=None if self.all_oracle_cards is None else
                           self.all_oracle_cards[i][j],
                           query=query,
                           table=self.table,
                           oracle_est=self.oracle)
                    if j % 100 == 0:
                        for est in estimators:
                            est.report(num_queries)
                self.save_fp.close()
            for est in estimators:
                err_fp = open('error.txt', 'w')
                for i, e in enumerate(est.errs):
                    err_fp.write(str(i) + '\t' + str(e) + '\n')
                err_fp.close()
                ...
                results[str(est) + '_max'] = est.errs[-1]
                results[str(est) + '_p99'] = est.errs[int(len(est.errs) * 0.99)]
                results[str(est) + '_p95'] = est.errs[int(len(est.errs) * 0.95)]
                results[str(est) + '_median'] = np.median(est.errs)
                est.report(num_queries)
