...
def main(argv):
    ...
    conn = pg.connect(dbname='zizhong_imdb', user='peizhi', port='5433')
    ...
    for t in join_tables:
        tables[t] = datasets.LoadImdb(t, data_dir='/home_nfs/peizhi/zizhong/dataset/imdb/job/', use_cols='content')
    ...
    if FLAGS.print_sel:
        ...
        sql_fp = open(FLAGS.output_sql, 'w')
        #next_template_idx = 3
...
#            queries.extend(
#                MakeQueries(cursor, num_queries_per_template,
#                            tables_in_templates, table_names, join_keys, rng))
            qs, cards, sqls = MakeQueries(cursor, num_queries_per_template,
                                    tables_in_templates, table_names, join_keys, rng)
            for sql in sqls:
                sql_tmp = sql.replace('\n', ' ')
                sql_fp.write(sql_tmp + '\n')
