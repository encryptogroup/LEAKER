...
def test_jackk_coeffs():
    ...
    assert j1[0] == approx(d)
    assert j2[0] == approx(d)
    assert j3[0] == approx(d)
    assert j4[0] == approx(d)

    assert j1[1] == approx((m - 1) / m)
    assert j2[1] == approx((2 * m - 3) / m)
    assert j3[1] == approx((3 * m - 6) / m)
    assert j4[1] == approx((4 * m - 10) / m)

    assert j1[2] == approx(0)
    assert j2[2] == approx(-(m - 2) ** 2 / (m * (m - 1)))
    assert j3[2] == approx(-(3 * m ** 2 - 15 * m + 19) / ((m - 1) * m))
    assert j4[2] == approx(-(6 * m ** 2 - 36 * m + 55) / ((m - 1) * m))

    assert j1[3] == approx(0)
    assert j2[3] == approx(0)
    assert j3[3] == approx((m - 3) ** 3 / ((m - 2) * (m - 1) * m))
    assert j4[3] == approx((4 * m ** 3 - 42 * m ** 2 + 148 * m - 175) / (m * (m - 1) * (m - 2)))

    assert j1[4] == j2[4] == j3[4] == approx(0)
    assert j4[4] == approx(-(m - 4) ** 4 / ((m - 3) * (m - 2) * (m - 1) * m))
